...
function renderVersionIndex(version, commands, isMarkdownPath) {
  ...
  for (const cmd of commands) {
    ...
    lines.push(`- [\`php artisan ${cmd.name}\`](https://artisan.eplus.dev/${version}/${slug}${markdownExtension})${desc}`)
  }
  lines.push('')
  lines.push(`Source: https://artisan.eplus.dev/${version}${isMarkdownPath ? '.md' : ''}`)
  return lines.join('\n')
}
...
function renderCommand(command, version, isMarkdownPath) {
  ...
  lines.push('')
  lines.push(`Source: https://artisan.eplus.dev/${version}/${slug}${isMarkdownPath ? '.md' : ''}`)
  return lines.join('\n')
}
